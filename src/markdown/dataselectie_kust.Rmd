...
Er is altijd onzekerheid op de exacte locatie van een waarneming. We zien een groot aantal waarnemingen met een accuraatheid groter dan 25:
...
We voegen eerst wetenschappelijke namen en taxonomische info toe door te verbinden met de [GBIF taxonomic backbone](https://www.gbif.org/dataset/d7dddbf4-2cf0-4f39-9b2a-bb099caae36c).
...
We vergelijken de Nederlandse soortnamen met de Bruyne et al. (2015). We passen de volgende namen aan (links: naam in dataset, rechts naam in de Bruyne et al. (2015)):  
...
final_df <- land_zoetwater_kust_selectie %>%
  select(id, datum = date, ned_soortnaam = `species name`, wet_soortnaam, 
         ...
         methode = method, telmethode = `counting method`, locatie = location, 
         latitude = lat, longitude = lng, lambert_x = `local x`, 
         lambert_y = `local y`, precisie = accuracy, notities = notes, 
         is_zeker = `is certain`, link, verbatim_name, species, author, genus, 
...
soorten_lijst <- final_df %>%
  distinct(ned_soortnaam, wet_soortnaam, species, author, genus, 
           family, phylum, kingdom) %>% 
  arrange(kingdom, phylum, family, wet_soortnaam)

write_csv(soorten_lijst, file = paste(out_path, "soortenlijst.csv", sep = "/"))

# Toon soortenlijst
soorten_lijst
```

We slaan ten slotte een lijst op van de waarnemingen niet gedetermineerd tot op soortniveau.

```{r}
ongedetermineerd <- final_df %>%
  filter(is.na(species))

write_csv(ongedetermineerd, file = paste(out_path, "ongedetermineerd.csv", 
                                         sep = "/"))
```
