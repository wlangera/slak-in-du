...
```{r}
# Laad data
slakken_data <- read_csv(paste0(data_path, "land_snails.csv"))

# Zet om naar sf object
slakken_data_sf <- st_as_sf(slakken_data, 
  coords = c("decimalLongitude", "decimalLatitude"), crs = 4326)

# Zet crs om naar project crs
slakken_data_sf <- st_transform(slakken_data_sf, crs = proj_crs)
...
```{r}
#plot alle punten en selectie in 2 kleuren maar zoom in op studiegebied
ggplot() +
  geom_sf(data = studiegebied_kust_merged, fill = "white") +
  coord_sf(datum = proj_crs) +
  mytheme
