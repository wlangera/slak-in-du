...
test <- lapply(soortenlijst, function(i) {
  counts <- stats_utm_grid(data = slakken_data_sf_buffer, soortnaam = i, 
                           resolutie = 10) %>%
    st_drop_geometry() %>% 
    count(occurrence) %>% 
  ...
  list(i, aanwezig / (aanwezig + afwezig))
})


df <- do.call(what = rbind.data.frame, args = test)
names(df) <- c("ned_soortnaam", "prop_utm_10")

df %>% arrange(desc(prop_utm_10)) %>%
  kable()
